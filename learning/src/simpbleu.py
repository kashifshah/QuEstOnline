...
flags.DEFINE_boolean("clip_len", True, "clip length")
...
#################################################################################################
    ...
    #recall
    result["guess"] = [max(0,min(reflen)-k+1) for k in xrange(1,n+1)]
#################################################################################################
    ...
    for (ngram, count) in counts.iteritems():
#################################################################################################
        #result["correct"][len(ngram)-1] += refmaxcounts.get(ngram,0)
...
class Bleu(object):
    ...
    def __init__(self, test=None, refs=None, option="average", n=3, special_reflen=None):
        ''' singular instance '''
    ...
    def _single_reflen(self, reflens, option=None, testlen=None):
        ...
        if option == "shortest":
            ...
            reflen = float(sum(reflens))/len(reflens) if len(reflens) != 0 else 100
        elif option == "closest":
...
#geo###################################################################################################
        ...
        #    bleu *= float(totalcomps['correct'][k] + addprec) \
        #            / (totalcomps['guess'][k] + addprec + small)            
        #bleu = bleu ** (1./n)
#arith###################################################################################################
        for k in xrange(n):
...
###################################################################################################
        ...
        #ratio = (self._testlen + addprec + tiny) / (self._reflen + addprec + small) ## N.B.: avoid zero division
        #print ratio
        ...
        self._score = bleu
        self._ratio = ratio
        return self._score,totalcomps['guess'][0]
...
class Obtain_bleu:
    def __init__(self, testfile, reffile):
        self.testfile=testfile
        self.reffile=reffile

    def bleu_sentence(self,senttest, sentref,ap=0):
        bleus = Bleu(option=FLAGS.eff_ref_len)
        testlines = []
        testlines.append(senttest)
        refs = []
        refs.append([sentref])
        for i, (testline, ref) in enumerate(zip(testlines, refs), 1):
            ...
            bleus += (testline, ref)
        bleu,l = bleus.fscore(option=FLAGS.eff_ref_len, addprec=ap)
        return bleu,l
    def bleu_score(self):
        infile = fileinput.input(self.testfile) # "-" OK
        reffilenames = self.reffile
        bleus = Bleu(option=FLAGS.eff_ref_len)
        testlines = []
        ...
        ## STEP 1: READ IN REFS
        refs = []
        for i, lines in enumerate(itertools.izip(*[file(filename) for filename in self.reffile]), 1):
            refs.append(lines)
