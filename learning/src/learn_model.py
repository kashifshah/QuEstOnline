...
def set_optimization_params(opt):
    ...
    for key, item in opt.items():
        ...
        #params[key] = np.arange(item[0], item[1], item[2])
...
def fit_predict(config, X_train, y_train, X_test=None, y_test=None):
    ...
    if transformer is not None:
        ...
        log.info("X_train dimensions before fit_transform(): %s,%s" % X_train.shape)
        log.info("y_train dimensions before fit_transform(): %s" % y_train.shape)
        
        
        X_train = transformer.fit_transform(X_train, y_train)
        
        log.info("Dimensions after fit_transform(): %s,%s" % X_train.shape)
        log.info("Dimensions after fit_transform(): %s" % y_train.shape)
    ...
    joblib.dump(estimator, 'filename.pkl')
    f = open('/Users/kashif/Documents/projects/quest-master/learning/data/features/cpredictions_wmt12', 'w')
...
# s = select_features(estimator, X_train, y_train)
#    print s
...
def fit_predict_only(config, x_test=None, y_test=None):
    ...
    if (x_test is not None):
        log.info("Predicting unseen data using the trained model...(from previously learned model)")
        y_hat = estimator.predict(x_test)
        log.info("Evaluating prediction on the test set...")
        for scorer_name, scorer_func in scorers:
            v = scorer_func(y_test, y_hat)
